...
def fetch_policy_data(year, codes):
    ...
    if df is None or df.empty:
        print("  ⚠️  No policy data returned")
        return pd.DataFrame()
...
def compute_cohort_split(policy_df, impact_scores=None):
    ...
    print("  ⚠️  Cohort column empty — deriving from normalized_score (lower = higher risk)")
...
def compute_improvement_pivot(comp, year_codes):
    ...
    if not codes_in_csv:
        print("  ⚠️  No managed care product codes found in comparison CSV")
    ...
    if set(effective_codes) != set(year_codes):
        print(f"  ℹ️  Requested year codes: {year_codes}")
        print(f"  ℹ️  Codes found in CSV:   {effective_codes}")
        print(f"  ℹ️  Using codes found in CSV for pivot calculation")
...
def compute_zero_appt(comp, policy_df, year):
    ...
    if policy_df.empty:
        ...
        print(f"  ℹ️  policy_df empty — using {len(enrolled):,} MC users from CSV as enrolled")
    else:
...
def build_summary(year):
    ...
    if enrolled == 0:
        print(f"  ℹ️  No enrolled users found in d_policy for {year} codes — displaying 0")
