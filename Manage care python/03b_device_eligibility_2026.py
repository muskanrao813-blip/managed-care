...
def get_engine():
    ...
    if _ENG is None:
        _ENG = create_engine(
            f'trino://{TRINO_USER}:{_pw}@{TRINO_HOST}:{443}/systemxhttp_scheme=https'
        )
    return _ENG
...
def assign_device(row):
    ...
    Diabetes users:
      HbA1c > 8     CGM  |  HbA1c 6.58  Glucometer
...
def calc_score(row):
    """
    Scoring per orchestration doc (0100 total):
      Clinical Risk  (30%): morbidity count  raw 0100, 0.30 = 030 pts
      Engagement     (25%): consultations from claims (025 raw), 0.25/25100 = 025 pts
                            App opens not available (0), consultations as recommended = 15 pts raw
      Adherence      (25%): claim service types used as proxy for logs (025 raw), 0.25 = 025 pts
      Lifestyle      (20%): HRA risk factors (020 raw), 0.20/20100 = 020 pts
    """
    ...
    # Raw: app_opens (010, not available=0) + consultations (015)
    ...
    # 4. Lifestyle Risk  HRA factors (already 020 scale)
    lifestyle_raw = 0
